  ...
  'Environmental protected sites': [
    {
      ...
      userAdjustableParameters: [
        { label: 'Distance from layer', type: 'number', default: 2 },
      ],
    },
    {
...
const LayerControlPanel = ({ mapRef }: LayerControlPanelProps) => {
  ...
  const defaultExpanded = Object.entries(layers).find(
              ...
              return (
                <Box key={item.name} className="layer-item">
                  <label htmlFor={checkboxId}>
                    ...
                    <Checkbox
                      id={checkboxId}
                      checked={checkedLayers[item.name]}
                      onChange={() => handleCheckboxChange(item.name)}
                    />
                  </label>
                  <IconButton
                    size="small"
                    onClick={() => openProps(item.name)}
                  >
              ...
              Object.values(layers)
                ...
                .userAdjustableParameters.map(param => (
                  <TextField
                    ...
                    onChange={e =>
                      handleParamChange(param.label, e)
                    }
                    slotProps={{
                      input: {
                        endAdornment: (
